...
Histogram3D = tuple[np.ndarray[tuple[int, int, int], np.dtype[np.float64]], list[np.ndarray[tuple[int], np.dtype[np.float64]]]]
...
def _dA_d(nr_d, dA) -> Vector_d:
    return nr_d*dA

dA_d =  DerivedSurfaceFunc(
    _dA_d,
    (nr_d, "dA"),
    "surf_element",
    )
...
def radial_projection(vec: (str | SurfaceFunc[Vector_u])) -> DerivedSurfaceFunc[Scalar]:
    _vec = get_func(vec)
    return DerivedSurfaceFunc(_scalar_prod, (_vec, nr_d), f"{_vec.name}_r")
...
def criterion(
    u_t: str | None | SurfaceFunc[Scalar],
    ) -> SurfaceFunc[Scalar]:
    if u_t is None:
        return no_crit
    _u_t = get_func(u_t)
    return DerivedSurfaceFunc(_crit, (_u_t, ), f"Crit({_u_t.name})")
...
def integrate_flux(
    ...
    funcs = (_flux, dA_d, criterion(crit))
    if weight is not None:
        _weight = get_func(weight)
        funcs += (_weight, )
        name = f"Integral({_flux.name}*{_weight.name})"
    return DerivedSurfaceFunc(_surf_int, funcs, name, out=out)
...
def mass_histogram(
    ...
    **kwargs
    ...
    return DerivedSurfaceFunc(_hist, (mass_flux, dA_d, _func, c, 'dt'), name, out=out, **kwargs)
...
def get_many(funcs: Iterable[SurfaceFunc]):
    return DerivedSurfaceFunc(_return_tuple, funcs, ", ".join(f.name for f in funcs))
...
tau = DerivedSurfaceFunc(_tau, ('r', radial_projection(vel)), 'tau',)
...
def _hist3d(
    ...
    **kwargs
    ...
    if out:
        f_r = np.clip(f_r, 0, None)
    sample = np.stack([f.flatten() for f in (ye, s, tau)], axis=1)
    return np.histogramdd(sample, weights=f_r.flatten(), **kwargs)
...
def ejecta(
    ...
    **kwargs
    ...
    return DerivedSurfaceFunc(
        _hist3d,
        ('passive_scalars.r_0', 'hydro.aux.s', tau_b(target_temp, eos),
         mass_flux, dA_d, criterion(crit), 'dt'),
        name, out=out, **kwargs)

################################################################################

def mdot(**kwargs) -> SurfaceFunc[float]:
    return integrate_flux(mass_flux, **kwargs)
...
nu0_e = DerivedSurfaceFunc(truediv, (nu0_lum, nu0_nlum), 'e_nu0')
